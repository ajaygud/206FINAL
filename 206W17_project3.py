...
def get_user_tweets(userhandle):
	t = "twitter_" + userhandle

	if t in CACHE_DICTION:
		results = CACHE_DICTION[t]
		return(results[0:20])
	else:
		user_timeline_results = api.user_timeline(userhandle) 
		twenty_results = user_timeline_results[0:20]
		CACHE_DICTION[t] = twenty_results
		cache_file = open(CACHE_FNAME, 'w') 
		cache_file.write(json.dumps(CACHE_DICTION))
		cache_file.close()
		return (twenty_results)
...
#NOT NULL, FOREIGN KEY (user_id) REFERENCES Users(user_id) ON UPDATE SET NULL
...
for i in list_of_screen_names:
	umich_users = api.get_user(i)
	blah = (umich_users["id_str"], umich_users["screen_name"], umich_users["favourites_count"], umich_users["description"])
...
tup = zip(tweet_id_list, tweet_text_list, umich_user_id_list, tweet_time_list, tweet_retweet_list)
for i in tup:
	cur.execute('INSERT INTO Tweets VALUES (?,?,?,?,?)', i)
conn.commit()

#query_0 = "INSERT INTO Tweets(user_id) SELECT user_id FROM Users"
#cur.execute(query_0)
...
query_2 = "SELECT screen_name FROM Users"
cur.execute(query_2)
screen_name_results = cur.fetchall()
screen_names = [r[0] for r in screen_name_results]
...
query_3 = "SELECT * FROM Tweets WHERE retweets > 25"
cur.execute(query_3)
...
query_4 = "SELECT description FROM Users WHERE num_favs > 5"
cur.execute(query_4)
description_results = cur.fetchall()
descriptions_fav_users = [r[0] for r in description_results]
...
query_5 = "SELECT screen_name, Tweets.text FROM Users INNER JOIN Tweets ON Tweets.user_id = Users.user_id WHERE Tweets.retweets > 50"  #N
joined_result = list(cur.execute(query_5))
...
query_6 = "SELECT screen_name, Tweets.text FROM Users INNER JOIN Tweets ON Tweets.user_id = Users.user_id" 
cur.execute(query_6)
joined_result_dict = cur.fetchall()

#new_default_dic = {i:j for i,j in joined_result_dict}

default_dic = collections.defaultdict(list)
for i,j in joined_result_dict:
	default_dic[i].append(j)

twitter_info_diction = dict(default_dic)
