...
def personal_map(function_object, list_object):
	empty_list = []
	k = map(function_object, list_object)
	for item in k:
...
n_and_p = zip(names, prod_list)
names_and_productivities = list(n_and_p)
...
def readfiles(list_of_filenames):	
	for f in list_of_filenames:
		...
		current_file.close()
# Define len_check
def len_check(generator_file_lines):
	return(line for line in generator_file_lines if len(line) > 40)

# Define main_filterer
def main_filterer(list_of_filenames):
	return list(len_check(readfiles(list_of_filenames)))
